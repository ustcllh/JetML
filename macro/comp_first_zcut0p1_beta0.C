...
int comp_first_zcut0p1_beta0(){
  ...
  TString cut = "deltag>0 && zg>0";

  // pythia
  TString inf1 = "../results/ptmin80/pythia_zcut0p1_beta0.root";
  TFile* f1 = new TFile(inf1, "READ");
  TTree* tr_pythia = (TTree*) f1->Get("jet");

  tr_pythia->Draw("zg>>h_z_pythia(10, 0, 0.5)", cut.Data(), "");
  TH1F* h_z_pythia = (TH1F*) gDirectory->Get("h_z_pythia");
  h_z_pythia->Sumw2();
  h_z_pythia->Scale(1./0.05/h_z_pythia->GetEntries());

  tr_pythia->Draw("deltag>>h_delta_pythia(9, 0, 0.45)", cut.Data(), "");
  ...
  TH1F* frame11 = DrawFrame(0., 0.5, 1, 18, title1, false);
  ...
  Tl.SetTextFont(43);
  Tl.SetTextSize(30);
  Tl.DrawLatex(0.04,16,"p_{T,jet}> 80GeV, anti-k_{T}, R=0.4");
  Tl.DrawLatex(0.04,14.5,"Soft Drop z_{cut}=0.1, #beta=0.");
  Tl.DrawLatex(0.04,13, "First Soft Drop");
  ...
  c1->SaveAs("../plot/z_first_zcut0p1_beta0.pdf");
  ...
  TH1F* frame21 = DrawFrame(0., 0.42, 2e-1, 5e3, title2, false);
  ...
  Tl.DrawLatex(0.04,2e3,"p_{T,jet}> 80GeV, anti-k_{T}, R=0.4");
  Tl.DrawLatex(0.04,8e2,"Soft Drop z_{cut}=0.1, #beta=0.");
  Tl.DrawLatex(0.04,3e2, "First Soft Drop");
  ...
  p22->Draw();
  p22->cd();
  TH1F* frame22 = DrawFrame(0., 0.42, -0.2, 3.2, "; #Delta; #frac{Model}{Pythia8}", false);
  ...
  c2->SaveAs("../plot/delta_first_zcut0p1_beta0.pdf");
